...
//Add Button Code
    if(isset($_POST['add'])){ //If POST Name Add
      ...
      if(empty($_POST['nombre'])){
        echo"<script>
        alert('El NOMBRE del platillo es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else if(empty($_POST['price'])){
        echo"<script>
        alert('El PRECIO del platillo es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else if(empty($_POST['descript'])){
        echo"<script>
        alert('La DESCRIPCION del platillo es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else{
        ...
        $query = "INSERT INTO especialidades(nombre,precio,descripcion) VALUES('$name','$price','$descript')";
        $run = mysqli_query($connection, $query);

        //If Run Happens, Shows Successful Message
        if($run){
            echo "<script>
              alert('La especialidad ha sido ingresada');
            </script>";
        }else{ //If Not Shows Error Message To User
          echo "<script>
          alert('La especialidad no ha sido ingresada, INTENTELO nuevamente');
        </script>";
        }
      }
    }
...
//Delete Button Functionality
    if(isset($_POST['delete'])){//If Click on Delete Button
      ...
      if(empty($_POST['id'])){
        echo"<script>
        alert('El ID del platillo es necesario para ELIMINAR, INTENTELO nuevamente');
      </script>";
      }else{
        //Variable Inicialization Inside If
              ...
              //If Executes Correctly Shows Succesful Message To User
                if($run){
                    echo "<script>0
                      alert('La especialidad ha sido eliminada');
                    </script>";
                }else{//If Not Error Message
                  echo "<script>
                  alert('La especialidad no ha sido eliminada, INTENTELO nuevamente');
                </script>";
                }
      }
  ...
  if(isset($_POST['modify'])){//If Click on Delete Button
    if(empty($_POST['id'])){
      echo"<script>
      alert('El ID del platillo es necesario para MODIFICAR, INTENTELO nuevamente');
    </script>";
    }else{
    //Variable Inicialization Inside If
    $id = $_POST['id'];
    ...
    $query = "UPDATE especialidades SET NOMBRE = '$name', precio = '$price', descripcion = '$descript' WHERE ID_ESPECIALIDAD = '$id'";
    //Run With Query & Connection Variable
    $run = mysqli_query($connection,$query);
    ...
    //If Executes Correctly Shows Succesful Message To User
      if($run){
          echo "<script>0
            alert('La especialidad ha sido actualizada');
          </script>";
      }else{//If Not Error Message
        echo "<script>
        alert('La especialidad no ha sido actualizada, INTENTELO nuevamente');
      </script>";
      }
    }
}
...
<div class="container">
  ...
  <!--Form-->
<form class="mt-5" action="especialidades.php" method="post">
<div class="mb-3">
    <label for="exampleInputEmail1" class="form-label" >ID Especialidad</label>
