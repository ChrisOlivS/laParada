...
//Add Button Code
    if(isset($_POST['add'])){ //If POST Name Add
      ...
      if(empty($_POST['nombre'])){
        ...
        $query = "INSERT INTO arroces(nombre_a,precio_a,descripcion_a) VALUES('$name','$price','$descript')";
        $run = mysqli_query($connection, $query);

        //If Run Happens, Shows Successful Message
        if($run){
            echo "<script>
              alert('El arroz ha sido ingresado');
            </script>";
        }else{ //If Not Shows Error Message To User
          echo "<script>
          alert('El arroz no ha sido ingresado, INTENTELO nuevamente');
        </script>";
        }
      }
    }
...
//Delete Button Functionality
    if(isset($_POST['delete'])){//If Click on Delete Button
      ...
      if(empty($_POST['id'])){
        echo"<script>
        alert('El ID del arroz es necesario para ELIMINAR, INTENTELO nuevamente');
      </script>";
      }else{
        //Variable Inicialization Inside If
              ...
              $query = "DELETE FROM arroces WHERE ID_ARROZ = '$id'";
              ...
              //If Executes Correctly Shows Succesful Message To User
                if($run){
                    echo "<script>0
                      alert('El Arroz ha sido eliminado');
                    </script>";
                }else{//If Not Error Message
                  echo "<script>
                  alert('El Arroz no ha sido eliminado, INTENTELO nuevamente');
                </script>";
                }
      }
  ...
  if(isset($_POST['modify'])){//If Click on Delete Button
    if(empty($_POST['id'])){
      echo"<script>
      alert('El ID del arroz es necesario para MODIFICAR, INTENTELO nuevamente');
    </script>";
    }else{
    //Variable Inicialization Inside If
    $id = $_POST['id'];
    ...
    $query = "UPDATE arroces SET NOMBRE_A = '$name', precio_a = '$price', descripcion_a = '$descript' WHERE ID_ARROZ = '$id'";
    //Run With Query & Connection Variable
    $run = mysqli_query($connection,$query);
    ...
    //If Executes Correctly Shows Succesful Message To User
      if($run){
          echo "<script>0
            alert('El Arroz ha sido actualizado');
          </script>";
      }else{//If Not Error Message
        echo "<script>
        alert('El Arroz no ha sido actualizado, INTENTELO nuevamente');
      </script>";
      }
    }
}
...
<div class="container">
  ...
  <!--Form-->
<form class="mt-5" action="rice.php" method="post">
<div class="mb-3">
    <label for="exampleInputEmail1" class="form-label" >ID Arroz</label>
