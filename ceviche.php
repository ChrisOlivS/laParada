...
//Add Button Code
    if(isset($_POST['add'])){ //If POST Name Add
      ...
      if(empty($_POST['nombre'])){
        echo"<script>
        alert('El NOMBRE del ceviche es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else if(empty($_POST['price'])){
        echo"<script>
        alert('El PRECIO del ceviche es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else if(empty($_POST['descript'])){
        echo"<script>
        alert('La DESCRIPCION del ceviche es un campo necesario, INTENTELO nuevamente');
      </script>";
      }else{
        ...
        $query = "INSERT INTO ceviches(nombre_c,precio_c,descripcion_c) VALUES('$name','$price','$descript')";
        $run = mysqli_query($connection, $query);

        //If Run Happens, Shows Successful Message
        if($run){
            echo "<script>
              alert('El ceviche ha sido ingresada');
            </script>";
        }else{ //If Not Shows Error Message To User
          echo "<script>
          alert('El ceviche no ha sido ingresada, INTENTELO nuevamente');
        </script>";
        }
      }
    }
...
//Delete Button Functionality
    if(isset($_POST['delete'])){//If Click on Delete Button
      ...
      if(empty($_POST['id'])){
        echo"<script>
        alert('El ID del ceviche es necesario para ELIMINAR, INTENTELO nuevamente');
      </script>";
      }else{
        //Variable Inicialization Inside If
              ...
              $query = "DELETE FROM ceviche WHERE ID_CEVICHE = '$id'";
              ...
              //If Executes Correctly Shows Succesful Message To User
                if($run){
                    echo "<script>0
                      alert('El ceviche ha sido eliminado');
                    </script>";
                }else{//If Not Error Message
                  echo "<script>
                  alert('El ceviche no ha sido eliminado, INTENTELO nuevamente');
                </script>";
                }
      }
  ...
  if(isset($_POST['modify'])){//If Click on Delete Button
    if(empty($_POST['id'])){
      echo"<script>
      alert('El ID del ceviche es necesario para MODIFICAR, INTENTELO nuevamente');
    </script>";
    }else{
    //Variable Inicialization Inside If
    $id = $_POST['id'];
    ...
    $query = "UPDATE ceviche SET NOMBRE_C = '$name', precio_C = '$price', descripcion_C = '$descript' WHERE ID_CEVICHE = '$id'";
    //Run With Query & Connection Variable
    $run = mysqli_query($connection,$query);
    ...
    //If Executes Correctly Shows Succesful Message To User
      if($run){
          echo "<script>0
            alert('El ceviche ha sido actualizado');
          </script>";
      }else{//If Not Error Message
        echo "<script>
        alert('El ceviche no ha sido actualizado, INTENTELO nuevamente');
      </script>";
      }
    }
}
...
<div class="container">
  ...
  <!--Form-->
<form class="mt-5" action="ceviche.php" method="post">
<div class="mb-3">
    <label for="exampleInputEmail1" class="form-label" >ID Ceviche</label>
